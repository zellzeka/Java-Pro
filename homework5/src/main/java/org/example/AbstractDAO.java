...
public abstract class AbstractDAO<T> {
 ...
 /* Завдання 2 та 3 Реалізація виводу в консоль данних з колонок, що вказані при запиті, та додавання в поле id
 обєкту автоінкрементованого значення з таблиці
 *   */
            try (Statement st = conn.createStatement()) {
                st.execute(sql, Statement.RETURN_GENERATED_KEYS);
                try (ResultSet key = st.getGeneratedKeys()){
                    if (key.next()){
                        int requiredKey = key.getInt(1);
                        id.setAccessible(true);
                        id.set(t, requiredKey);
                    }
                }
            }
    ...
    public List<T> getAll(Class<T> cls, String... params) {
        ...
        int paramQuantity = params.length;
